...
class ClassificationModels:
    ...
    def lSTM(self):
        ...
        model.add(LSTM(32, return_sequences=True,
               input_shape=(X_train1.shape[1], X_train1.shape[2])))  # returns a sequence of vectors of dimension 32
        ...
        history=model.fit(X_train1, Y_train, batch_size=64, epochs=50, validation_data=(X_test1, Y_test))
        
        score, acc = model.evaluate(X_test1, Y_test, batch_size=70)
       
        Y_Probability = model.predict(X_test1)
        Y_Pred=[]
        for i,j in enumerate(Y_Probability):
            l=np.where(j==max(j))[0][0]
            Y_Pred.append(l)
        Cm_Y1=[]
        for i in Cm_Y:
            Cm_Y1.append(i[0])
...
Y[:,0]=labelcoder.fit_transform(Y[:,0])
...
'''
T2=Object1.kNN_Classification()
T3=Object1.randomForestClassifier()
T4=Object1.naiveBayesClassifier()
T5=Object1.xgBoostClassifier()

#################################################################################################################################
import seaborn as sns                     
import matplotlib.pyplot as plt
fig, ax = plt.subplots(figsize=(6,6))
sns.heatmap(T5[1], annot=True, fmt='d',
            xticklabels=["Down","No","Up"], yticklabels=["Down","No","Up"])'''
